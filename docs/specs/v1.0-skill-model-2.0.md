...
interface Skill {
  ...
  ttsText: string;          // what we read out via speechSynthesis (implementation may use `tts`)
  timerSeconds: number;     // interval for TTS reminders, must be >= 1 (implementation may use `interval`)

  heroId?: HeroId | null;   // null = general skill (implementation may use heroIds: HeroId[] for multi-select)
  roleId?: RoleId | null;   // null = general skill (implementation may use roleIds: RoleId[] for multi-select)
...
- `owPractice.v1.categories` – array of categories (optional for V1.0 if categories defer).
- `owPractice.v1.tags` – array of tags (optional for V1.0 if tags defer).
- `owPractice.v1.settings` – user-local settings (optional for V1.0; filter/sort may be session-based).
...
- **Filter/sort persistence**: Per product decision, filter and sort preferences may be **session-based** (in-memory only) for V1.0; persistence is not required. If not persisted, `UserSettingsV1` or equivalent can live in React state only.
...
- When a user changes filters or sort options:
  ...
  - Persistence is optional; filter/sort may be session-based for V1.0.

#### 5.2. Skill Creation & Editing

- **Creating a new skill**:
  - Start with `gameId = 'overwatch'`.
  - Optionally pre-fill `heroId`/`heroIds` and `roleId`/`roleIds` from the current filters if there is a single value selected; otherwise leave them null/empty.
  - New skills are `isPreset = false`, `isArchived = false`.
  - `createdAt` and `updatedAt` should be set to the current time (ISO string).
  - Enforce `timerSeconds` / `interval` >= 1 to avoid timer edge cases.
...
- Empty lists (no skills after filters):
 ...
 - Partially corrupted or outdated `UserSettingsV1` (if persisted):
...
### 9. Built-in Preset Skills (V1.0)

Built-in presets should include **general** skills (no hero/role) and **hero/role-specific** skills to demonstrate filtering.

**General presets** (no `heroId`/`roleId` or empty arrays):
- Off Angle, Target Priority, Ult Tracking (or equivalent).

**Hero-specific preset**:
- **Kiriko – TP then Suzu**
  - `heroIds: ['kiriko']` (or equivalent)
  - Name: "TP then Suzu"
...
### 10. Post-QA Updates (from qa-v1.0-skill-model-review.md)

The following clarifications and fixes were added after QA review. Engineers should implement these.

**Load logic (P0 – data loss bug)**  
- Only use built-in presets when `storedSkills.length === 0`.  
- If all skills are archived (`visibleSkills.length === 0` but `storedSkills.length > 0`), keep the stored skills. Do not overwrite with presets.

**General skills behavior**  
- When filters are active, General skills (no hero/role) are excluded.  
- When no filters are applied, General skills are shown.

**Empty filtered state**  
- Show "No skills match your filters" when `filteredSkills.length === 0` and `skills.length > 0`. Optionally include "Clear filters" / "Add skill" prompts.

**Interval validation**  
- Enforce `timerSeconds` / `interval` >= 1 to avoid timer TTS spam.
