...
```ts
type GameId = 'overwatch2' | string; // allow other games later
...
- For now, you can hardcode `gameId: 'overwatch2'` for all built-in skills and user skills.
...
interface Skill {
  id: SkillId;
  gameId: GameId;           // always 'overwatch2' for now
...
interface Category {
  ...
  gameId: GameId;       // future-proofing; for now 'overwatch2'
  createdAt: string;
  updatedAt: string;
}
...
interface UserSettingsV1 {
  selectedGameId: GameId;           // 'overwatch2' for now
